...
class TableMatcher:
    ...
    def __call__(self, structure_strs, structure_bboxes, dt_boxes, rec_res):
        if self.filter_ocr_result:
            dt_boxes, rec_res = self._filter_ocr_result(
                structure_bboxes, dt_boxes, rec_res)
    ...
    def match_result(self, dt_boxes, structure_bboxes):
        ...
        for i, gt_box in enumerate(dt_boxes):
            ...
            for j, pred_box in enumerate(structure_bboxes):
                ...
                distances.append((distance(gt_box, pred_box), 1. - compute_iou(
                    gt_box, pred_box)))  # compute iou and l1 distance
            ...
            if '</td>' in tag:
                ...
                if td_index in matched_index.keys():
                    ...
                    if '<b>' in ocr_contents[matched_index[td_index]
                                             [0]] and len(
                                                 matched_index[td_index]) > 1:
                        b_with = True
                        end_html.extend('<b>')
                    for i, td_index_index in enumerate(
                            matched_index[td_index]):
                        ...
                        if len(matched_index[td_index]) > 1:
                            ...
                            if len(content) == 0:
                                continue
                            if i != len(matched_index[td_index]
                                        ) - 1 and ' ' != content[-1]:
                                content += ' '
