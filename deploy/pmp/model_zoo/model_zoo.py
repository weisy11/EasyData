...
from pmp.utils.download import get_config_path, get_model_path
...
TASK_DICT = {
    ...
    # TODO
    # system
    'PP-OCRv3': 'paddlecv://configs/test_keypoint.yml',
}


def list_model(filters=[]):
    model_zoo_file = pkg_resources.resource_filename('pmp.model_zoo',
                                                     MODEL_ZOO_FILENAME)
...
def get_config_file(task):
    ...
    if task not in TASK_DICT:
        ...
        logger.error("Illegal task: {}, please use one of {}".format(
            task, tasks))
    path = TASK_DICT[task]
