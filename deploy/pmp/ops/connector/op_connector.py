...
__all__ = [
    ...
    "KeyFrameExtractionOp", "TableMatcherOp"
]
...
@register
class ClsCorrectionOp(BaseConnector):
    """
    rotate
    """

    def __init__(self, cfg, class_num=2):
        super().__init__()
...
@register
class BboxCropOp(BaseConnector):
    """
    BboxCropOp
    """
...
@register
class PolyCropOp(BaseConnector):
    """
    PolyCropOp
    """
...
@register
class FragmentCompositionOp(BaseConnector):
    """
...
@register
class KeyFrameExtractionOp(BaseConnector):
    """
    ...
    def __init__(self, cfg):
        ...
        self.input_keys = cfg["Inputs"]
        self.check_input_keys()
        assert cfg["algo"] in ["luv_diff", ]
        if cfg["algo"] == "luv_diff":
            self.extractor = LUVAbsDiffKeyFrameExtractor(cfg["params"])
...
@register
class TableMatcherOp(BaseConnector):
    """
    TableMatcherOp
    """
    ...
    def __init__(self, cfg):
        ...
        self.input_keys = cfg["Inputs"]
        self.check_input_keys()
        filter_ocr_result = cfg.get("filter_ocr_result", False)
    ...
    def __call__(self, inputs):
        outputs = []
        for idx in range(len(inputs)):
            structure_strs = inputs[idx][0]
            structure_bboxes = inputs[idx][1]
            dt_boxes = inputs[idx][2]
            rec_res = inputs[idx][3]
